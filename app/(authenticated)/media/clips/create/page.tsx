  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch("/clips", {
        method: "POST",
        body: JSON.stringify({ title, author, videoUrl, thumbnailUrl, isVisible }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 w-full max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Nouveau clip</h1>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enchanted Tools ouvre ses portes…" />
        </div>
        <div className="space-y-2">
          <Label>Auteur *</Label>
          <Input required value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Miroki_Lover32" />
        </div>
      </section>

      <section className="space-y-4">
        <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wide">Vidéo (verticale) *</h2>
        {videoFile ? (
          ...
          <DropZone accept="video/*" onFile={setVideoFile}>
            <div className="flex flex-col items-center gap-3 py-10 text-muted-foreground">
              ...
              <p className="text-sm font-medium">Vidéo verticale (Reels)</p>
              <p className="text-xs">Format 9:16 recommandé</p>
            </div>
          </DropZone>
        )}
      </section>
      ...
      <section className="space-y-4">
        ...
        {thumbnailPreview ? (
          <div className="relative w-32 rounded-lg overflow-hidden border">
            <button type="button" onClick={() => { setThumbnailFile(null); setThumbnailPreview(null); }}
              className="absolute top-2 right-2 z-10 rounded-full bg-background/80 p-1">
              <X className="size-4" />
            </button>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={thumbnailPreview} alt="" className="w-full aspect-[9/16] object-cover" />
          </div>
        ) : (
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button type="submit" disabled={loading}>
          {loading && <Spinner />}
          {loading ? "Enregistrement…" : "Créer le clip"}
        </Button>
      </div>
    </form>
  );
